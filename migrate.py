...
def create_org(url, token, owner_user, org, full_name='', description='', website='', location=''):
    print 'Organization will be created -> On Git server: ' + url + \
        ', Owner user: ' + owner_user + ', Organization: ' + org
    ...
    print 'Organization has been created -> On Git server: ' + \
        url + ', Owner user: ' + owner_user + ', Organization: ' + org
...
def copy_org(src_url, src_token, dst_url, dst_token, dst_owner_user, org):
    print 'Organization migration has been started -> From:' + src_url + ' To: ' + \
        dst_url + ' Destination owner user: ' + dst_owner_user + ' Organization: ' + org

    orgJson = get_org(src_url, src_token, org)
    if orgJson == '':
        raise Exception(
            'Organization [' + org + '] does not exist on ' + src_url)
    ...
    print 'Organization migration has been completed -> From:' + src_url + ' To: ' + \
        dst_url + ' Destination owner user: ' + dst_owner_user + ' Organization: ' + org
...
def copy_orgs(src_url, src_token, dst_url, dst_token, dst_owner_user):
    ...
    for org in orgs:
        ...
        copy_org(src_url, src_token, dst_url,
                 dst_token, dst_owner_user, orgName)
...
def create_repo(url, token, org, repo, src_repo, dst_token):
    ...
    if get_repo(url, token, org, repo) != '':
        raise Exception(
            'Repository [' + repo + '] of organization [' + org + '] already exists on ' + url)
...
def migrate_repo(src_url, src_token, src_owner, dst_url, dst_token, dst_owner, repo):
    ...
    src_repo_url = get_token_inserted_url(
        src_url, src_token) + '/' + src_owner + '/' + repo + '.git'
    dst_repo_url = get_token_inserted_url(
        dst_url, dst_token) + '/' + dst_owner + '/' + repo + '.git'

    subprocess.check_call(
        ['git', '-c', 'http.sslVerify=false', 'clone', '--bare', src_repo_url])
    os.chdir(repo_dir)
    subprocess.check_call(
        ['git', '-c', 'http.sslVerify=false', 'push', '--mirror', dst_repo_url])
    os.chdir('..')
    subprocess.check_call(['rm', '-rf', repo_dir])
...
def copy_repo(src_url, src_token, src_org, dst_url, dst_token, dst_org, repo):
    ...
    if get_org(src_url, src_token, src_org) == '':
        raise Exception(
            'Organization [' + src_org + '] does not exist on ' + src_url)
...
def copy_repos(src_url, src_token, dst_url, dst_token):
    ...
    for org in orgs:
        ...
        for repo in repos:
            repoName = repo['name']
            copy_repo(src_url, src_token, orgName, dst_url,
                      dst_token, orgName, repoName)
...
def copy_user_repo(src_url, src_token, src_user, dst_url, dst_token, dst_user, repo):
    ...
    if src_repo == '':
        raise Exception(
            'Repository [' + repo + '] of user [' + src_user + '] does not exist on ' + src_url)
    if src_repo['private']:
        print 'User repository [' + repo + \
...
def copy_user_repos(src_url, src_token, src_user, dst_url, dst_token, dst_user):
    ...
    for repo in repos:
        repoName = repo['name']
        copy_user_repo(src_url, src_token, src_user, dst_url,
                       dst_token, dst_user, repoName)
...
parse_arguments()
copy_orgs(arg_src_url, arg_src_token, arg_dst_url,
          arg_dst_token, arg_dst_owner_user)
